...
# @File    : cityscapes_sfnet_single_gpu_trainner.py
# @IDE: PyCharm
"""
SFNet trainner for cityscapes dataset
"""
import os
...
class SFNetCityScapesTrainer(object):
    """
    init sfnet single gpu trainner
    """

    def __init__(self, cfg):
        """
        initialize sfnet trainner
        """
        self._cfg = cfg
        ...
        self._model_save_dir = ops.join(
            self._cfg.TRAIN.MODEL_SAVE_DIR, self._model_name)
        self._tboard_save_dir = ops.join(
            self._cfg.TRAIN.TBOARD_SAVE_DIR, self._model_name)
        ...
        if self._enable_miou:
            with tf.variable_scope('miou'):
                ...
                indices = tf.squeeze(
                    tf.where(tf.less_equal(gt, self._cfg.DATASET.NUM_CLASSES - 1)), 1)
        ...
        with tf.variable_scope(name_or_scope='train_step'):
            ...
            if self._optimizer_mode == 'sgd':
                ...
                raise ValueError(
                    'Not support optimizer: {:s}'.format(self._optimizer_mode))
            optimize_op = optimizer.minimize(
                self._loss, var_list=train_var_list)
        ...
        with tf.variable_scope('summary'):
            ...
            model_params_file_save_path = ops.join(
                self._tboard_save_dir, self._cfg.TRAIN.MODEL_PARAMS_CONFIG_FILE_NAME)
        ...
        LOG.info('Initialize cityscapes sfnet trainner complete')
    ...
    def train(self):
        ...
        for epoch in range(epoch_start_pt, self._train_epoch_nums):
            ...
            if epoch % self._snapshot_epoch == 0:
                if self._enable_miou:
                    snapshot_model_name = 'cityscapes_train_miou={:.4f}.ckpt'.format(
                        train_epoch_mious)
                    ...
                    snapshot_model_name = 'cityscapes_train_loss={:.4f}.ckpt'.format(
                        train_epoch_losses)
                    snapshot_model_path = ops.join(
                        self._model_save_dir, snapshot_model_name)
                    os.makedirs(self._model_save_dir, exist_ok=True)
