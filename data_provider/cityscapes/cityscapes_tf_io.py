...
class _CityScapesTfWriter(object):
        ...
        with tf.python_io.TFRecordWriter(tfrecords_path) as writer:
            for sample_path in sample_image_paths:
                ...
                gt_image_raw = tf.gfile.FastGFile(
                    gt_src_image_path, 'rb').read()

                # prepare gt binary image
                gt_binary_image_raw = tf.gfile.FastGFile(
                    gt_label_image_path, 'rb').read()
    ...
    def write_tfrecords(self):
        ...
        train_tfrecords_file_path = ops.join(
            self._tfrecords_dir, train_tfrecords_file_name)
        ...
        val_tfrecords_file_path = ops.join(
            self._tfrecords_dir, val_tfrecords_file_name)
        self._write_example_tfrecords(
...
class _CityScapesTfReader(object):
    ...
    def __init__(self, dataset_flag):
        ...
        if self._dataset_flags not in ['train', 'val']:
            raise ValueError(
                'flags of the data feeder should be \'train\', \'val\'')
    ...
    def __len__(self):
        ...
        :return:
        """
        tfrecords_file_paths = ops.join(
            self._tfrecords_dir, 'cityscapes_{:s}.tfrecords'.format(self._dataset_flags))
        assert ops.exists(tfrecords_file_paths), '{:s} not exist'.format(
